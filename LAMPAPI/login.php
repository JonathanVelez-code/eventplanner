<?php
    ...
    else
	{
		...
		if( $row = $result->fetch_assoc()  )
		{
			...
			returnWithInfo( $row['firstName'], $row['lastName'], $row['userID'], $row['email'], $row['university'] );
		}
		else
		{
	...
	{
		$retValue = '{"id":' . $userID . ',"firstName":"' . $firstName . '","lastName":"' . $lastName . '","email":"' . $email . '", "univerity":"' . $univerity . '","error":""}';
		sendResultInfoAsJson( $retValue );
	}
